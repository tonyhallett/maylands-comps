import { Box, IconButton, SvgIcon, lighten } from "@mui/material";
import { CSSProperties } from "react";

interface BatSVGProps {
  rubberFillColor: CSSProperties["fill"]; // #FF4F28
  bladeFillColor1: CSSProperties["fill"]; // #FDD083
  bladeFillColor2: CSSProperties["fill"]; // #F3A25A
  showBall?: boolean;
  flip?: boolean;
}

interface BatButtonProps extends BatSVGProps {
  enabled: boolean;
  clicked: () => void;
}

export function BatButton({ enabled, clicked, ...svgProps }: BatButtonProps) {
  return (
    <Box m={1}>
      <IconButton
        style={{ color: "white" }}
        disabled={!enabled}
        onClick={clicked}
      >
        <BatSVG {...svgProps} />
      </IconButton>
    </Box>
  );
}

function BatSVG({
  rubberFillColor,
  showBall = true,
  bladeFillColor1,
  bladeFillColor2,
  flip = false,
}: BatSVGProps) {
  return (
    <SvgIcon>
      <svg
        version="1.1"
        xmlns="http://www.w3.org/2000/svg"
        x="0px"
        y="0px"
        viewBox="0 0 128 128"
        transform={flip ? "scale(-1, 1)" : undefined}
      >
        <path
          style={{ fill: bladeFillColor1 }}
          d="M40.77,61.97c0,0-9.24,3.88-8.98,4.97c0.26,1.1,1.87,3.68,2.84,5.88c0.69,1.57,2,4.26,1.74,6.59
    s-1.55,6.46-1.55,6.46l13.83,11.69c0,0,2.53-2.72,6.51-3.94c3.87-1.19,6.8-0.44,8.28-0.32c2.52,0.19,6.2,0.52,6.72,0.26
    c1.09-0.54-14.41-20.54-14.41-20.54L40.77,61.97z"
        />
        <path
          style={{ fill: bladeFillColor2 }}
          d="M42.04,73.01c0,0-8.24,13.38-13.61,20.94s-9.95,13.12-10.6,13.83c-0.65,0.71-4.07,3.49-3.88,5.04
    c0.19,1.55,4.01,4.07,7.82,6.78c3.81,2.71,6.98,5.56,8.4,5.04s5.33-7.62,7.75-11.11c4.5-6.48,21.13-29.89,21.13-29.89L42.04,73.01z"
        />
        <path
          style={{ fill: bladeFillColor1 }}
          d="M41.01,82.38c0.39,0,0.15,3.3,3.18,5.17c3.04,1.87,5.41,1.42,5.74,1.81
    c0.14,0.16-3.36,5.24-7.43,11.05c-5.21,7.44-10.99,15.29-11.5,16.09c-0.71,1.1-2.84,2.91-5.11,1.49c-2.23-1.4-4.63-3.24-5.47-4.6
    c-0.84-1.36,0.04-2.64,1.33-4.32c0.71-0.92,5.81-7.93,10.79-14.93C36.64,88.4,40.69,82.38,41.01,82.38z"
        />

        {/* the rubber !*/}
        <path
          style={{ fill: rubberFillColor }}
          d="M31.79,66.95l36.68,26.79c0,0,10.24,1.25,17.06-2.22s14.5-8.67,21.91-22.39
    c7.66-14.19,14.72-40.22-10.53-56.33s-47.77,3.95-54.35,11.73s-12.64,17-13.17,28.01C28.9,62.48,31.79,66.95,31.79,66.95z"
        />

        {showBall && (
          <>
            {/* the ball */}

            {/* ball shadow - original #EE3E23*/}
            <path
              style={{ fill: lighten(rubberFillColor, 0.2) }}
              d="M53.72,47.69c11.17-0.01,18.01-9.86,16.71-21.03S55.27,14.27,55.27,14.27L37.34,31.3l-1.22,1.87
    C36.11,33.18,37.05,47.71,53.72,47.69z"
            />

            <path
              style={{ fill: "#D8CDC9" }}
              d="M40.19,7.01c-6.37,4.69-9.29,14.75-3.99,22.74c4.69,7.07,16.74,10.43,25.19,3.18
    c8.87-7.61,5.78-19.65,1.14-24.29C56.33,2.45,46.38,2.45,40.19,7.01z"
            />
            <path
              style={{ fill: "#EAEBEB" }}
              d="M41.66,9.95C37.5,14.02,36.44,22.01,41.74,26c4.92,3.71,11.66,3.26,15.49-1.3
    c3.99-4.75,2.2-12.88-1.55-15.41C52.64,7.25,46.23,5.46,41.66,9.95z"
            />
            <path
              style={{ fill: "#FEFFFF" }}
              d="M40.67,23.37c1.86-0.19,2.1-1.84,2.51-5.02c0.38-2.94,3.25-6.41,5.76-6.97
    c2.71-0.6,4.3-0.63,4.29-2.34c-0.02-2.03-4.01-3.61-8.56-1.56c-4.62,2.08-6.97,7.62-6.97,11.24C37.7,22.41,38.89,23.55,40.67,23.37z
    "
            />
          </>
        )}
      </svg>
    </SvgIcon>
  );
}
